...
public class CoinSorter extends JFrame {
	...
	public void selectionSort() {
		for (int i=0; i<strip.size(); i++) {
			...
			for(int j = i+1 ; j<strip.size(); j++){
				if(compare(j, min, PAUSE_DELAY) <= 0){
					min = j;
				}
			}
			swap(i, min, PAUSE_DELAY);
	...
	public void insertionSort() {
		for (int i=0; i< strip.size(); i++) {
			for(int j = i; j>0; j--){
				if(compare(j, j-1, PAUSE_DELAY) <= 0){
					swap(j, j-1, PAUSE_DELAY);
				}
				else{
					break;
				}
	...
	protected int partition(int lo, int hi) {
		...
		int i = lo-1;
        for(int j = lo; j < hi; j++ ){
			if(j<=hi && compare(j, hi, PAUSE_DELAY) <= 0) {
				i++;
				swap(i, j, PAUSE_DELAY);
            }
		}

        i++;
		swap(i, hi, PAUSE_DELAY);
        	
        return i;    
	}

	protected void qSort(int lo, int hi) {
		if (lo < hi) {
